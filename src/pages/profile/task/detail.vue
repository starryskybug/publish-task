...
function handleTerminateTask() {
    loadingRef.value = true
    api.GET(`/api/wx/task/terminate/${task.value.id}`).then(resp => {
        uni.showToast({
            title: resp.msg,
            icon: "success"
        })
...
<template>
    <view>
        ...
        <!-- 状态栏 -->
        <view 
            class="py-14rpx text-center mt-20rpx font-bold"
            :style="{ backgroundColor: getStatusBarStyle().bg, color: getStatusBarStyle().text }"
        >
            <view class="text-36rpx">{{ getStatusText() }}</view>
            <view v-if="task.status === 1" class="text-28rpx">{{ timer }}</view>
        </view>
        ...
        <view class="px-30rpx py-40rpx">
            ...
            <view v-if="task.type === 'big'" class="mt-48rpx">
                <view class="w-full h-12rpx bg-#CCE4FF rounded-full overflow-hidden">
                    <view 
                        class="h-full bg-#006E6B"
                        :style="{ width: `${(task.completed / task.total) * 100}%` }"
                    ></view>
                </view>
                <view class="text-right text-24rpx text-#006E6B mt-12rpx">
            ...
            <view class="px-10rpx mt-76rpx">
                <textarea
                    class="border-2 border-solid border-#006E6B rounded-30rpx p-34rpx text-32rpx text-black font-bold w-auto"
                    disabled
                    :value="task.url"
                />
            ...
            <view v-if="!task.myTask && task.status === 1" class="mt-82rpx shadow p-30rpx border border-solid border-#EAEAEA rounded-30rpx">
                ...
                <!-- 上传状态提示 -->
                <view v-if="loadingRef" class="mt-20rpx text-center text-28rpx text-#FF9500">
                    正在上传图片...
                </view>
